...
function initConfig() {
    ...
    const logPath = process.env.LOG_FILE_PATH?.toUpperCase();
    const logErrorPath = process.env.LOG_FILE_ERROR_PATH?.toUpperCase();
    const logPathFile = configFileData?.logPath?.toUpperCase();
    const logErrorPathFile = configFileData?.logErrorPath?.toUpperCase();
    ...
    if (logErrorPathFileFinal) {
        config.formatter = logErrorPathFileFinal;
    }
