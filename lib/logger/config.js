...
const defaultConfig = {
    ...
    networkPort: constants.network.PORT,
    networkHostname: constants.network.HOSTNAME
}
...
function initConfig() {
    ...
    const networkPort = process.env.NETWORK_PORT?.toLowerCase();
    const networkHostname = process.env.NETWORK_HOSTNAME?.toLowerCase();
    const networkPortFile = configFileData?.networkPort?.toLowerCase();
    const networkHostnameFile = configFileData?.networkHostname?.toLowerCase();
    ...
    const networkPortFinal = networkPort || networkPortFile;
    const networkHostnameFinal = networkHostname || networkHostnameFile;
    ...
    if (networkPortFinal) {
        config.networkPort = networkPortFinal;
    }

    if (networkHostnameFinal) {
        config.networkHostname = networkHostnameFinal;
    }
